...
pub(crate) struct ResizeHandle<T: 'static, E: 'static + Render> {
    ...
    /// When set, the handle stays draggable but paints nothing.
    invisible: bool,
...
impl<T: 'static, E: 'static + Render> ResizeHandle<T, E> {
    ...
    /// Keep the handle draggable but stop it painting its 1px line.
    ///
    /// Useful when the adjacent panels already draw their own borders: the
    /// handle's line runs the full length of the split and would otherwise
    /// extend past those panels' rounded corners.
    pub(crate) fn invisible(mut self, invisible: bool) -> Self {
...
impl<T: 'static, E: 'static + Render> Element for ResizeHandle<T, E> {
    ...
    fn request_layout(
        ...
        window.with_element_state(id.unwrap(), |state, window| {
            ...
            // An invisible handle still highlights while it is being dragged,
            // so the split stays discoverable without painting a permanent line.
